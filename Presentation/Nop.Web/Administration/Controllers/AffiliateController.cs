    ...
    public partial class AffiliateController : BaseAdminController
    {
            ...
            bool prepareEntireAddressModel = true)
        {
            ...
            if (prepareEntireAddressModel)
            {
                ...
                model.AvailableAffiliates.Add(new SelectListItem { Text = "无", Value = "0" });

                foreach(var a in _affiliateService.GetAllAffiliates())
                {
                    if(a.Level==1 || a.Level==2)
                    {
                        model.AvailableAffiliates.Add(new SelectListItem {Text=a.GetFullName(),Value=a.Id.ToString() });
                    }
                }
            }
        }
        ...
        public ActionResult AffiliatedOrderList(int affiliateId)
        {
            ...
            model.AvailableSubAffiliates.Add(new SelectListItem { Text="无",Value="0" });
            var a = _affiliateService.GetAffiliateById(affiliateId);
            if(a!=null &&　a.Level<3)
            {
                var alla = _affiliateService.GetAllAffiliates();
                var subids = new List<int?>();
                foreach(var asub in alla.Where(af=>af.ParentAffiliateId ==affiliateId))
                {
                    subids.Add(asub.Id);
                    model.AvailableSubAffiliates.Add(new SelectListItem { Text = asub.GetFullName(), Value = asub.Id.ToString() });
                }
                if(a.Level==1 && subids.Count>0)
                {
                    foreach (var asub in alla.Where(af => subids.Contains( af.ParentAffiliateId)))
                    {
                        model.AvailableSubAffiliates.Add(new SelectListItem { Text = asub.GetFullName(), Value = asub.Id.ToString() });
                    }
                }
        ...
        [ChildActionOnly]
        public ActionResult AffiliatedCustomerAndOrderList(int affiliateId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return Content("");
        ...
        public ActionResult AffiliatedOrderList(DataSourceRequest command, AffiliatedOrderListModel model)
        {
            ...
            var orders = _orderService.SearchOrders(
                ...
                ps: paymentStatus,
                ss: shippingStatus, 
                affiliateId: model.SubAffiliateId==0 ? affiliate.Id: model.SubAffiliateId,
